...
import { saveVideoToDB } from "@/utils/indexedDB";
import PremiumModal from "./PremiumBox";
...
const VideoInfo = ({ video }: any) => {
  ...
  useEffect(() => {
    ...
    const { payment_success, session_id } = router.query;

    if (payment_success === "true" && session_id) {
      setPaymentSessionId(session_id as string);
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      try {
        await axiosInstance.post("/payment/check-limit", { userId: user._id });
      } catch (error: any) {
      ...
      await saveVideoToDB(video._id, response.data);

      await axiosInstance.post("/downloads/save", {
        userId: user._id,
        videoId: video._id,
      });
      ...
      toast.success("Download Complete", {
        id: toastId,
        description: "Video saved to your offline downloads.",
        duration: 4000,
      });
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Avatar className="w-10 h-10">
            <AvatarFallback>{video.videochanel ? video.videochanel[0] : 'U'}</AvatarFallback>
          </Avatar>
          <div>
