...
const Header = () => {
  ...
  useEffect(() => {
    if (user) {
      ...
      console.log("Is Premium?", user.planType);
    }
  }, [user]);
...
{user?.planType && user?.planType != "Free" && (
  <div 
    className="flex items-center gap-1 bg-gradient-to-r from-yellow-100 to-amber-100 px-3 py-1.5 rounded-full border border-yellow-300 mr-2 shadow-sm"
    title={`Premium Member (${user.planType})`}
  >
